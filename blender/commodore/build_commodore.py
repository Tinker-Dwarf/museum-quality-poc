...
def build():
    ...
    face = cube("Face", (31.45, 0, 7.05), (2.2, 7.6, 11.4), shroud)
    bevel(face, 1.05, 6)
    parent(face, root)
    crown = cylinder("Crown", (31.45, 0, 12.55), 0.42, 7.2, shroud, rot=(math.pi / 2, 0, 0), verts=24)
    parent(crown, root)
    for s in (-1, 1):
        wrap = cube(f"FaceWrap_{s}", (30.2, s * 3.70, 7.4), (3.6, 0.35, 10.4), shroud)
        bevel(wrap, 0.18, 2)
        parent(wrap, root)
        grab = cube(f"FaceGrab_{s}", (32.55, s * 2.6, 8.2), (0.08, 0.08, 3.2), steel)
        parent(grab, root)
        marker = sphere(f"Marker_{s}", (32.35, s * 3.45, 12.15), 0.16, lamp, segs=10)
        parent(marker, root)
    cdoor = cube("CouplerDoor", (32.55, 0, 2.15), (0.12, 1.8, 1.1), dark)
    parent(cdoor, root)
