...
def build():
    ...
    face = cube("Face", (31.35, 0, 6.55), (2.8, 7.8, 13.2), shroud)
    bevel(face, 1.25, 6)
    parent(face, root)
    for s in (-1, 1):
        marker = sphere(f"Marker_{s}", (32.4, s * 3.35, 12.05), 0.14, lamp, segs=10)
        parent(marker, root)
    cdoor = cube("CouplerDoor", (32.7, 0, 1.85), (0.10, 1.6, 0.9), dark)
    parent(cdoor, root)
    ...
    for s in (-1, 1):
        for zi, z in enumerate((6.4, 7.6, 10.6)):
            ...
            parent(riv, root)
    bezel = cylinder("Bezel", (32.65, 0, 9.15), 0.82, 0.22, steel, verts=28)
    parent(bezel, root)
    lens = sphere("Lens", (32.78, 0, 9.15), 0.68, lamp, segs=24)
    parent(lens, root)
    ...
    parent(hring, root)
    grill = cube("GrillPanel", (32.62, 0, 12.55), (0.16, 1.8, 0.7), dark)
    parent(grill, root)
    for i in range(5):
        sl = cube(f"GrillSlat_{i}", (32.70, -0.64 + i * 0.32, 12.55), (0.08, 0.10, 0.55), steel)
        parent(sl, root)
    stack = cylinder("StackWell", (20.5, 0, BOIL_Z + BOIL_R - 0.05), 0.55, 0.22, dark, rot=(0, 0, 0), verts=24)
    parent(stack, root)
    whistle = cylinder("Whistle", (18.2, 0.45, BOIL_Z + BOIL_R + 0.35), 0.12, 0.7, steel, rot=(0, 0, 0), verts=10)
    parent(whistle, root)
    for i, ox in enumerate((-0.35, 0.0, 0.35)):
        pop = cylinder(f"PopValve_{i}", (17.4 + ox, 0, BOIL_Z + BOIL_R + 0.22), 0.10, 0.28, steel, rot=(0, 0, 0), verts=8)
        parent(pop, root)
    for s in (-1, 1):
        scoop = cube(f"StepScoop_{s}", (23.6, s * 4.55, 8.95), (1.8, 0.55, 0.35), dark)
        parent(scoop, root)
        pstep = cube(f"PilotStep_{s}", (33.2, s * 3.55, 1.55), (0.9, 0.7, 0.16), steel)
        parent(pstep, root)
    ...
    for s in (-1, 1):
        walk = cube(f"Walk_{s}", (8.0, s * 4.20, 8.55), (27.0, 0.85, 0.16), shroud)
        parent(walk, root)
        cover = cube(f"SideCover_{s}", (7.0, s * 4.62, 7.20), (24.0, 0.12, 2.5), shroud)
        parent(cover, root)
        fval = cube(f"FrontValance_{s}", (25.4, s * 4.50, 6.70), (7.2, 0.14, 3.8), shroud)
        parent(fval, root)
        rail = cube(f"WalkRail_{s}", (8.0, s * 4.68, 9.05), (27.0, 0.07, 0.08), steel)
        parent(rail, root)
        sw = cube(f"StairWell_{s}", (29.0, s * 3.82, 6.35), (3.8, 0.22, 5.6), dark)
        parent(sw, root)
        for i in range(5):
            st = cube(f"NoseStep_{s}_{i}", (29.4 - i * 0.45, s * 3.82, 3.9 + i * 0.95), (1.05, 0.70, 0.11), shroud)
            parent(st, root)
        cabst = cube(f"CabStep_{s}", (-8.2, s * 4.20, 8.15), (1.3, 0.80, 0.16), shroud)
        parent(cabst, root)

    frame = cube("Frame", (8.0, 0, 3.4), (36.0, 2.2, 0.7), iron)
    parent(frame, root)
    chin = cube("Chin", (31.9, 0, 1.70), (2.2, 6.4, 2.6), shroud)
    bevel(chin, 0.65, 5)
    parent(chin, root)
    pilot = cube("PilotBeam", (33.3, 0, 0.62), (0.8, 5.2, 0.32), iron)
    parent(pilot, root)
    fcoup = cube("FrontCoupler", (33.9, 0, 0.58), (0.45, 0.24, 0.24), steel)
    parent(fcoup, root)
